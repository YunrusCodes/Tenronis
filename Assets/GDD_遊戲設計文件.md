...
- **空**：可放置方塊
- **已佔用**：有方塊存在
- **特殊屬性**：方塊有不同類型（Normal, Void, Explosive）
...
- **效果**：在底部插入一整行普通方塊（BlockType.Normal）
- **出現關卡**：Stage 12+
- **特性**：
  - 直接佔據底部空間
  - 可用消行清除
...
- **效果**：在底部插入一整行虛無方塊（BlockType.Void）
- **出現關卡**：Stage 15+（高級關卡）
- **特性**：
  - 虛無方塊不可被子彈摧毀
  - 只能通過消行清除
...
| AreaDamage | Stage 6 | Stage 6+ | 區域清除威脅 |
| AddBlock | Stage 8 | Stage 8+ | 增加堆疊壓力 |
| AddExplosiveBlock | Stage 10 | Stage 10+ | 雙重傷害威脅 |
| InsertRow | Stage 12 | Stage 12+ | 空間壓迫 |
| InsertVoidRow | Stage 15 | Stage 15+ | 火力抵銷 |
| CorruptExplosive | Stage 15 | Stage 15+ | 高難度腐化 |
| CorruptVoid | Stage 17 | Stage 17+ | 最高難度腐化 |
...
遊戲採用**主題系統**，支援多個主題，每個主題包含**三種難度軌道**（Casual、Standard、Expert）。每個難度軌道包含10個關卡。

**主題系統特點**：
- 使用 StageSetSO 管理主題，每個主題包含三種難度的關卡列表
- 玩家先選擇主題，再選擇難度，每個主題10關
- 支援擴展多個主題，提供更豐富的遊戲內容
- 每個主題可以有獨特的敵人、故事和視覺風格
- **專案現況**：已建立10個主題，每主題3種難度×10關，共300個關卡配置

**難度軌道系統**：
- **Casual（休閒）**：35秒目標擊殺時間，較慢子彈速度（6格/秒）
- **Standard（標準）**：25秒目標擊殺時間，中等子彈速度（8格/秒）
- **Expert（專家）**：20秒目標擊殺時間，快速子彈（10格/秒），啟用智能瞄準

### 範例：主題系統說明

專案支援**多個主題**，每個主題包含**10個關卡**，每個關卡有**3種難度軌道**（Casual, Standard, Expert）。

**專案現況**：
- 已建立10個主題
- 每主題3種難度 × 10關 = 30個關卡配置/主題
- 總計：10主題 × 30關卡 = **300個關卡配置**

### 關卡參數範例（某主題 - Standard難度軌道的前10關）

| 關卡 | 敵人名稱範例 | Boss | HP | 射擊間隔 | 子彈速度 | 獎勵 | 主要子彈類型 |
|------|---------|------|-----|---------|---------|------|------------|
| 1 | 初級威脅 | - | 175 | 1.9s | 8 | 1 | Normal |
| 2 | 低級敵人 | - | 350 | 1.8s | 8 | 1 | Normal |
| 3 | 巡邏者 | - | 525 | 1.7s | 8 | 1 | Normal |
| 4 | 強化體 | - | 700 | 1.6s | 8 | 1 | Normal |
| 5 | 小頭目 | - | 875 | 1.5s | 8 | 1 | Normal |
| 6 | 進階威脅 | - | 1050 | 1.4s | 8 | 1 | Normal, Area |
| 7 | 複雜敵人 | - | 1225 | 1.3s | 8 | 1 | Normal, Area |
| 8 | 強力威脅 | - | 1400 | 1.2s | 8 | 1 | Normal, Area, AddBlock |
| 9 | 精英敵人 | - | 1575 | 1.1s | 8 | 1 | Normal, Area, AddBlock |
| 10 | 主題Boss | ⭐ | 1750 | 1.0s | 8 | 1 | Normal, Area, AddBlock, AddExplosive |

**說明**：
- **Boss**：⭐標記的關卡為Boss關卡，難度和獎勵更高
- **獎勵**：擊敗後獲得的Buff選擇次數
- **Corrupt×**：包含CorruptExplosive和CorruptVoid
- **難度差異**：相同關卡在不同難度軌道有不同的數值（HP、射速、子彈速度）
- **Auto Balance**：使用 StageDataSO 的自動平衡功能根據 PDA 和 SP 計算數值

### 關卡設計理念

每個主題包含**10個關卡**，可分為**多個階段**，逐步引入新機制和挑戰。以下是一個主題的階段範例，適用於任何主題（根據 StageDataSO 的 EnableSkillsByStageProgression() 自動設置）：

#### 第一階段（Stage 1-5，適用於所有難度軌道）：基礎入門 🌊
- **目標**：讓玩家熟悉基礎操作和核心機制
- **難度差異**：
  - Casual：射擊間隔較長（2.5-3.5s），子彈速度慢（6格/秒）
  - Standard：射擊間隔中等（1.8-2.2s），子彈速度中等（8格/秒）
  - Expert：射擊間隔較短（1.5-1.8s），子彈速度快（10格/秒）
- **子彈類型解鎖**：Normal
- **新機制**：
  - 基礎方塊堆疊
  - 消除發射導彈
...
- **目標**：引入多樣化攻擊模式
- **難度曲線**：
  - Stage 6：引入AreaDamage（範圍傷害）
  - Stage 8：引入AddBlock（添加方塊）
  - Stage 10：引入AddExplosiveBlock（爆炸方塊威脅）
- **子彈類型解鎖**：AreaDamage → AddBlock → AddExplosiveBlock
- **新機制**：
  - 區域傷害需要方塊佈局策略
  - 爆炸方塊雙重威脅
- **升級建議**：Burst、Counter、TacticalExpansion

#### 第三階段（Stage 10，主題Boss）：主題頭目 👹
- **目標**：主題最終挑戰
- **特色**：
  - 每個主題的最終Boss
  - 整合該主題所有機制
...
- **觸發條件**：擊敗敵人完成關卡
- **獲得規則**：
  - 每個關卡的 `rewardBuffCount` 決定獲得的Buff數量
  - 通常為 1 個Buff，Boss關卡可能獲得更多
  - 關卡完成後，如果有待選Buff，會進入升級選單
...
**關卡開始時自動恢復機制**：
- **觸發時機**：擊敗敵人完成關卡後，進入下一關時自動觸發
- **恢復量**：50% 最大 HP（向下取整）
- **適用範圍**：第二關及之後的所有關卡（第一關不觸發）
- **執行順序**：CP 恢復 → HP 恢復 → 關卡索引 +1
- **設計目的**：
  - 不佔用傳奇強化槽位
  - 鼓勵玩家積極推進關卡
  - 簡化 Buff 選擇策略
...
遊戲共有**9種Buff**，分為**6種普通強化**和**3種傳奇強化**。另有**2種主動技能**（Execution、Repair）需透過「戰術擴展」傳奇強化解鎖。
...
**效果**：解鎖技能（Execution和Repair）
- **起始等級**：0
- **最大等級**：2
- **Lv1效果**：解鎖處決技能（Execution，消耗5 CP）
- **Lv2效果**：解鎖修補技能（Repair，消耗30 CP）
- **累積效果**：逐步解鎖技能

**解鎖機制**：
- 初始狀態：技能未解鎖，無法使用
- Lv1：解鎖處決技能（按鍵1，消耗5 CP）
- Lv2：解鎖修補技能（按鍵2，消耗30 CP）
...
**重要**：Execution和Repair不是獨立的Buff，而是通過TacticalExpansion解鎖的技能
...
傳奇強化不使用權重系統，當普通強化達到滿級時，從3種傳奇強化中隨機選擇（若數量≤3則全部顯示）：

- **Defense**（裝甲強化）：無上限，生存流核心
- **Volley**（協同火力）：無上限，火力流核心
- **TacticalExpansion**（戰術擴展）：最高2級，解鎖技能

**注意**：Execution 和 Repair 是消耗CP的主動技能，需要通過「戰術擴展」傳奇強化解鎖後才能使用。
...
### 資源建立 ✅
- [x] 關卡ScriptableObject（**專案現況：10主題×30關卡配置=300個**）
- [x] 10個主題StageSetSO
- [x] 12個Buff ScriptableObject（6普通+4傳奇+2技能）
- [ ] 音效資源（8+ clips）
- [ ] 音樂資源（2 tracks）
- [x] 方塊精靈圖
- [x] 彈藥精靈圖（8種子彈類型）
...
2. **策略深度**：9種Buff（6普通+3傳奇）+ 8種敵人子彈類型 + 2種主動技能
3. **豐富內容**：多主題系統（已建立10主題×30關卡=300個關卡配置）
4. **高重玩性**：Roguelike隨機升級系統 + 三種難度軌道
5. **技術優秀**：清晰的架構和優化
6. **易於擴展**：支援多主題擴展，為未來內容預留空間
7. **生存機制**：關卡開始自動恢復HP，鼓勵積極推進
...
**更新內容**（v2.2）：
- 移除 Heal 傳奇強化，改為關卡開始時自動恢復機制
- 更新 Buff 總數：9種（6普通 + 3傳奇）
- 新增關卡開始自動恢復 50% HP 的內建機制
- 驗證並修正所有 Buff 相關描述與程式碼一致

**更新內容**（v2.1）：
- 修正Buff分類：TacticalExpansion為傳奇強化
- 明確Buff結構：6種普通強化 + 傳奇強化
- 更新權重系統：普通強化單獨計算權重
...
**專案狀態**：核心系統完成，已建立10主題×30關卡配置=**300個關卡配置**
